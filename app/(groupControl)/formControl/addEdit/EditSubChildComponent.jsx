...
function onSubmitFunctionCall(functionData, newState, formControlData, values, setStateVariable, childName, childIndex) {
  ...
  if (funcNameMatch && argsMatch !== null) {
    ...
    if (typeof func === "function") {
      ...
      console.log("setStateVariable", setStateVariable);
...
export default function EditSubChildComponent(props) {
  ...
  useEffect(() => {
    const transformedNewState = transformBooleanValues(newState);
  ...
  return (
    <React.Fragment>
      ...
      {openSubChildEdit && (
        <TableRow key={subChildObject._id}>
                  ...
                  {!isView && (
                    <div className="relative top-0 right-2 flex justify-end items-baseline ">
                      ...
                      <HoverIcon
                        ...
                        title={"Save"}
                        onClick={() => {
                          for (const field of subChild.fields) {
                            if (
                              field.isRequired &&
