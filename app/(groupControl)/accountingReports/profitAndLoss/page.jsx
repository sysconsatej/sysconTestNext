...
const ProfitAndLoss = () => {
  ...
  useEffect(() => {
    ...
    if (reportType === "S") {
      assets = groupByTb1GroupNameSummary(
        balanceSheetData.filter((item) => item.BalanceSheetName === "Expense")
      );
      liabilities = groupByTb1GroupNameSummary(
        balanceSheetData.filter((item) => item.BalanceSheetName === "Income")
      );
    } else if (reportType === "D") {
      assets = balanceSheetData
        .filter((item) => item.BalanceSheetName === "Expense")
        .sort((a, b) => a.tb1GroupName.localeCompare(b.tb1GroupName));
      liabilities = balanceSheetData
        .filter((item) => item.BalanceSheetName === "Income")
        .sort((a, b) => a.tb1GroupName.localeCompare(b.tb1GroupName));
    }

    // Calculate total balances for assets and liabilities
    const totalAssets = assets.reduce(
      (sum, asset) => sum + Math.abs(asset.closingBalance),
      0
    );
    const totalLiabilities = liabilities.reduce(
      (sum, liability) => sum + Math.abs(liability.closingBalance),
      0
    );
    ...
    return data
      .reduce((acc, current) => {
        ...
        const existingGroup = acc.find(
          (item) => item.tb1GroupName === groupName
        );
        if (existingGroup) {
          ...
          acc.push({
            ...current,
            closingBalance: Math.abs(current.closingBalance),
          });
        }
        return acc;
      }, [])
  ...
  async function handleExportToExcel(reportType) {
    ...
    const imageBuffer = await fetch(
      "http://94.136.187.170:3016/api/images/NCLP/nclpLogo20241111075655020.jpg"
    ...
    if (reportType === "S") {
      ...
      headerRow = worksheet.addRow(["Income", "Balance", "Expense", "Balance"]);
    } else {
      columns = [
        { key: "liability", width: 20 },
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      if (liability.tb1GroupName) {
        if (
          ...
          reportData.liabilityData[i - 1]?.tb1GroupName !==
            liability.tb1GroupName
        ) {
          liabilityStartRow = row.number;
        }

        const isEndOfLiabilityGroup =
          i === reportData.liabilityData.length - 1 ||
          reportData.liabilityData[i + 1]?.tb1GroupName !==
            liability.tb1GroupName;
        if (isEndOfLiabilityGroup) {
          merges.push({
            startRow: liabilityStartRow,
            endRow: row.number,
            column: 1,
          });
          liabilityStartRow = null;
      ...
      if (asset.tb1GroupName) {
        if (
          assetStartRow === null ||
          reportData.assetsData[i - 1]?.tb1GroupName !== asset.tb1GroupName
        ) {
          assetStartRow = row.number;
        }

        const isEndOfAssetGroup =
          i === reportData.assetsData.length - 1 ||
          reportData.assetsData[i + 1]?.tb1GroupName !== asset.tb1GroupName;
        if (isEndOfAssetGroup) {
          merges.push({
            startRow: assetStartRow,
            endRow: row.number,
            column: 4,
          });
          assetStartRow = null;
    ...
    const totalRow = worksheet.addRow({
      ...
      balanceLiability: reportData.totalLiabilities || 0.0,
      assets: "Total Expense",
      balanceAsset: reportData.totalAssets || 0.0,
    });
      ...
      <Accordion expanded={toggle} sx={{ ...parentAccordionSection }} key={1}>
        ...
        >
          ...
          >
            ...
            >
              <InputLabel sx={{ fontSize: "0.8rem" }} shrink>
                Branch
              </InputLabel>
              <Select
                value={year || ""}
                ...
                sx={{ fontSize: "0.7rem" }}
                displayEmpty
                renderValue={(selected) => selected || <em>Select Branch</em>}
              >
                <MenuItem value={38333} sx={{ fontSize: "0.7rem" }}>
                  COIMBATORE
                </MenuItem>
                <MenuItem value={38385} sx={{ fontSize: "0.7rem" }}>
                  DELHI
                </MenuItem>
                <MenuItem value={8331} sx={{ fontSize: "0.7rem" }}>
                  PUNE
                </MenuItem>
              </Select>
            </FormControl>
          ...
          <div className="flex mt-2">
            ...
            <button
              onClick={async () => {
                await handleExportToExcel(reportType);
              }}
