...
export default function RptIGM() {
  ...
  const MAX_CONTAINERS_PER_PAGE_MAIN = 10;
  ...
  const getLogoSrc = (logoPath) => {
    ...
    return `${baseUrlNext}${logoPath.startsWith("/") ? "" : "/"}${logoPath}`;
  };

  const CargoManifestTitle = ({ data }) => {
    const logoSrc = getLogoSrc(data?.PrincipalLogopath);

    return (
      <div
        className="flex items-center border-b border-black px-2 py-1 text-black"
      ...
      >
        <div className="flex items-center" style={{ width: "25%" }}>
          {logoSrc && (
            <img
              src={logoSrc}
              alt={data?.PrincipalName || "Principal logo"}
              style={{
                maxWidth: "120px",
                maxHeight: "38px",
                objectFit: "contain",
        ...
        <div
          ...
          style={{ width: "40%", fontSize: "8px" }}
        >
          <p className="font-bold uppercase">{data?.PrincipalName || ""}</p>
          <p className="whitespace-pre-line">{data?.AgentAddress || ""}</p>
        </div>
      </div>
    );
  };
    ...
    return (
      <>
        <div
          ...
          className="flex w-full p-1 pt-2 border-black border-b"
        >
          {/* Left Block – 25% */}
          <div style={{ width: "25%" }} className="flex">
            ...
            <div style={{ width: "65%" }}>{data?.blIssuePlaceText || ""}</div>
          </div>
          ...
          switch (reportId) {
            case "Freight Manifest Report":
              return data.map((dataItem, dataIdx) => {
                ...
                // Main page keeps fewer rows because the branded header uses top space.
                  ...
                  <React.Fragment key={`${dataItem.blNo || dataIdx}-wrap`}>
                    ...
                    >
                      <div
                        ...
                        style={{ height: "100%" }}
                      >
                        <CargoManifestTitle data={dataItem} />
                        <div>
