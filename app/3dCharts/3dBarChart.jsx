...
//       "position:absolute;pointer-events:none;background:rgba(135, 14, 215, 0.9);color:white;font-size:12px;padding:7px 11px;border-radius:7px;display:none;max-width:150px;text-align:center;line-height:1.5;z-index:20;";
...
//       const normH = (child.value / maxVal) * 1.5 + 0.16;
...
//         );
//       };
//       const col = new THREE.Color(child.color ? randomColor() : "#4c6ef5");
...
//       const topCol = col.clone().lerp(new THREE.Color(0xffffff), 0.25);
//       const top = new THREE.Mesh(
//         topGeo,
//         new THREE.MeshPhongMaterial({ color: topCol, shininess: 30 }),
//       );
...
//         "position:absolute;top:0;pointer-events:none;font-size:11px;font-weight:500;color:blue;text-align:center;white-space:nowrap;background:var(--color-background-primary);padding:2px 5px;border-radius:4px;border:0.5px solid var(--color-border-tertiary);opacity:0.92;max-width:88px;overflow:hidden;text-overflow:ellipsis;";
...
//       camera.position.y = Math.sin(rotX) * zoom + 1.5;
//       camera.position.z = Math.cos(rotY) * Math.cos(rotX) * zoom;
//       camera.lookAt(0, 1, 0);

//       bars.forEach((b, i) => {
//         const sp = project(b.x + 0.175, (b.normH + 0.18), 0.175);
//         const ld = labelRefs.current[i];
//         if (ld) {
//           ld.style.left = sp.x - 44 + "px";
...
export default function ThreeBarChart({ node, onDrillDown }) {
  ...
  useEffect(() => {
    ...
    const bars = children.map((child, i) => {
      ...
      const col = new THREE.Color(child.color ? getPaletteColor() : "#8B5CF6");
