...
const baseFunctionConfigCache = new WeakMap();
...
const customStyles = (showLabel) => {
  return {
    ...
    option: (base, state) => ({
      ...
      boxShadow: state.isFocused ? "inset 3px 0 0 var(--bg-color)" : "none",
    }),
...
function CustomeInputFields({
  ...
  //     console.log('field.controlDefaultValue',field.controlDefaultValue)
  //     if (field.controlDefaultValue != null && !values[field.fieldname]) {
  //       if (field.controlname.toLowerCase() === "dropdown") {
  //         defaultValues[field.fieldname] =
  //           field?.controlDefaultValue[0]?.value || field.controlDefaultValue;
  //           defaultValues[`${field.fieldname}dropdown`] =
  //           Number(field?.controlDefaultValue) || field.controlDefaultValue;
  //       } else {
  //         defaultValues[field.fieldname] =
  //           Number(field.controlDefaultValue) || field.controlDefaultValue;
  //       }
      ...
      {inputFieldData?.map((field, index, array) => {
        ...
        return (
          <React.Fragment key={index}>
            ...
            >
              <InputFieldRenderer
                key={`${field.controlname}_${field.fieldname}_${field?.id}_${index}`}
                field={field}
                index={index}
                fieldId={fieldId}
                // pass everything you currently use inside renderInputField:
                values={values}
                newState={newState}
                selectedIndex={selectedIndex}
                tableName={tableName}
                inputFieldData={inputFieldData}
                callSaveFunctionOnLastTab={callSaveFunctionOnLastTab}
                ...
                // plus any styling helpers you use:
                styles={styles}
                customStyles={customStyles}
                textInputStyle={textInputStyle}
                numberInputStyle={numberInputStyle}
                customDataPickerStyleCss={customDataPickerStyleCss}
                customTimePickerStyleCss={customTimePickerStyleCss}
                customDateTimePickerStyleCss={customDateTimePickerStyleCss}
                // and API function:
                dynamicDropDownFieldsData={dynamicDropDownFieldsData}
                dynamicFormData={dynamicFormData}
                clearFlag={clearFlag}
                // ✅ REQUIRED (these were causing no-undef)
                onValuesChange={onValuesChange}
                onKeyDown={onKeyDown}
                handleFieldValuesChange2={handleFieldValuesChange2}
                menuId={menuId}
                // ✅ REQUIRED (these were causing fileName/setFileName no-undef)
                fileName={fileName}
...
export default CustomeInputFields;
export { CustomeInputFields };
...
function InputFieldRenderer(props) {
  ...
  if (field && !baseFunctionConfigCache.has(field)) {
    baseFunctionConfigCache.set(field, {
      onChange:
        field?.functionOnChange ||
        field?.FunctionONchane ||
        field?.FunctionOnChange ||
        "",
      onBlur:
  ...
  const getTooltipValue = () => {
    ...
    const control = field.controlname.toLowerCase();
    if (control.includes("date")) {
      const parsed = dayjs(val);
      return parsed.isValid() ? parsed.format("DD/MM/YYYY") : val;
    }

    if (control.includes("time")) {
      const parsed = dayjs(val);
      return parsed.isValid() ? parsed.format("HH:mm:ss") : val;
    }

    switch (control) {
      case "dropdown":
        return dropDownValues?.find((item) => item.value == val)?.label || val;

      case "multiselect":
        return dropDownValues
          ?.filter((item) => val?.split(",")?.includes(item.value.toString()))
          ?.map((item) => item.label)
          ?.join(", ");
  ...
  const getFieldEventFunctionCalls = (eventType) => {
    if (!Array.isArray(field?.fieldsEvent)) return "";
    const normalizedEventType = String(eventType || "")
      .toLowerCase()
      .trim()
      .replace(/^on/, "");

    return field.fieldsEvent
      .filter((eventItem) => {
        const itemEventType = String(eventItem?.EventType || "")
          .toLowerCase()
          .trim();
        return itemEventType.replace(/^on/, "") === normalizedEventType;
      })
      .map((eventItem) => String(eventItem?.functionToCall || "").trim())
      .filter(Boolean)
      .join(";");
  };

  const mergeFunctionCalls = (...functionStrings) =>
    functionStrings
      .map((fnString) => String(fnString || "").trim())
      .filter(Boolean)
      .join(";");

  const baseFunctionConfig = field
    ? baseFunctionConfigCache.get(field) || {}
    : {};

  field.functionOnChange = mergeFunctionCalls(
    baseFunctionConfig?.onChange,
    getFieldEventFunctionCalls("onchange"),
  );
  field.functionOnBlur = mergeFunctionCalls(
    baseFunctionConfig?.onBlur,
    getFieldEventFunctionCalls("onblur"),
  );

  // ✅ KEEP THIS CHECK, but do it early:
  if (field?.isControlShow !== true) return null;
  ...
  useEffect(() => {
    ...
    const shouldFetchData =
      ...
      (field.controlname.toLowerCase() === "multiselect" &&
        field.dropDownValues?.length === 0 &&
        values[field.fieldname] !== undefined) ||
      (["dropdown", "multiselect"].includes(field.controlname.toLowerCase()) &&
  ...
  }, [values?.[field.fieldname]]); // Ensure all necessary dependencies are included

  useEffect(() => {
    if (firstRender.current && values?.[field.fieldname] !== undefined) {
      firstRender.current = false; // mark as false after the first render
      return; // exit early to prevent the code from running
    }
    if (
      isFocused == false &&
      values?.[field.fieldname] !== undefined &&
  ...
  function filterDropdownWithselectedValue(tableName, key) {
    const rows = Array.isArray(newState?.[tableName])
      ? newState[tableName]
      : [];
    ...
    return Array.from(uniqueValues).join(",") || "0";
  }
  function dynamicValuReplace(inputString) {
    ...
    for (let i = 0; i < variableNames.length; i++) {
      ...
      const patternMatch = variableName.match(/newState\.tblJob\[(\d+)\]\.id/);
  ...
  async function fetchData(
    ...
    // 2) Create NEW controller (local) + also store in state
    ...
    try {
      if (isNextPageNull && !["search", "first"].includes(action)) return false;

      // ✅ IMPORTANT: pass the NEW controller, not the old one
      const apiResponse = await dynamicDropDownFieldsData(
        requestData,
        newController,
      );

      if (apiResponse?.nextPage === null) setIsNextPageNull(true);
  ...
  const CustomMenuList = (props) => {
    const { innerRef, pageNo, setPageNo, setScrollPosition, scrollPosition } =
  ...
  function handleFuncChangeCall(
    ...
    if (funcNameMatch && (argsMatch !== null || (jsonMatch && jsonMatch[1]))) {
      ...
      if (typeof func === "function") {
        ...
        // if (updatedValues && typeof updatedValues.then === "function") {
        //   return updatedValues.then((resolvedValues) => {
        //     console.log("resolvedValues", funcName, resolvedValues);
        //     onChangeHandler(resolvedValues);
  ...
  const handleChangeDynamic = (value, field, switchToText) => {
    ...
    if (value && typeof value.format === "function") {
      formattedValue = dayjs(value).format("DD-MM-YYYY HH:mm:ss");
      updatedValues[`${field.fieldname}`] = formattedValue;
    }
  ...
  switch (field.controlname.toLowerCase()) {
    case "dropdown":
      return (
        <LightTooltip
          key={uniqueId}
          title={getTooltipValue() || field.yourlabel}
        >
          {field.isSwitchToText && switchToText ? (
            <CustomeTextField
              ...
              className={`lg:w-[12rem] ${styles.inputField}`}
              value={
                dropDownValues?.find(
              ...
              onBlur={(e) => {
                setIsFocused(false);
                if (onFocusValue !== e.target.value && e.target.value !== "") {
                  // values[field.fieldname] = e.target.value;
                  const funcCallString = field.functionOnChange;
                  if (
                    funcCallString !== undefined &&
            ...
            >
              ...
              <Select
                ...
                className={`${styles.inputField} ${inputWidthClass}`}
                menuPlacement="auto"
                components={{
                  MenuList: (props) => (
                    <CustomMenuList
                ...
                onBlur={async (e) => {
                  ...
                  if (onFocusValue !== values?.[field.fieldname]) {
                    ...
                    if (funcCallString) {
                      ...
                      // for (const funcCall of multiCallFunctions) {
                      //   if (funcCall.trim()) {
                      //     const nextValues = await Promise.resolve(
                      //       handleFuncChangeCall(
                      //         funcCall,
                      //         updatedValues,
                      //         field.fieldname,
        ...
        <LightTooltip title={getTooltipValue() || inputLabel}>
          <div className="relative ">
            ...
            <Select
              ...
              options={dropDownValues}
              menuPlacement="auto"
              className={`w-auto lg:min-w-[12rem]   ${styles.inputField}  `}
              components={{
                MenuList: (props) => (
                  <CustomMenuList
                    {...props}
                        ...
                        <Checkbox
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.checked) {
                              ...
                              if (!updatedValues.includes(item.id.toString())) {
          ...
          <CustomeTextField
            ...
            onChange={(e) => {
              ...
              if (!isInteger && field.typeValue == "number") {
                toast.warning(`${inputLabel} does not support decimal values`);
                return;
              }
      ...
      return (
        <LightTooltip title={getTooltipValue() || inputLabel}>
          <MuiColorInput
            ...
            ref={inputRef}
            label={
              <span className={`${styles.inputTextColor} `}>{inputLabel}</span>
            }
            onFocus={(e) => {
              setIsFocused(true);
              setonFocusValue(e.target.value);
    ...
    case "date":
      return (
        <LightTooltip
          key={uniqueId}
          title={isFocused ? "" : getTooltipValue() || inputLabel}
        >
          <div
            onBlur={() => {
              if (index == inputFieldData.length - 1) {
          ...
          >
            <DatePicker
              ...
              onChange={(date) => {
                if (
                  !canCommitPickerDate(date, field.fieldname, datePickerFormat)
                ) {
                  return;
                }
      ...
      return (
        <LightTooltip title={getTooltipValue() || inputLabel}>
          ...
          >
            ...
            <TextareaAutosize
              className={`resize w-full lg:w-[12rem] bg-[var(--inputBg)] disabled:border-[#B2BAC2] hover:border-[var(--inputBorderHoverColor)] disabled:text-[#B2BAC2] focus:outline-none focus:border-[var(--inputBorderHoverColor)] overflow-none h-[27px] text-[10px] pt-[6px] pl-[10px]  leading-normal rounded-xl rounded-br-none  border ${styles.inputField}`}
              ...
              onClick={() => {
                ...
                if (!/^https?:\/\//i.test(url)) {
                  ...
                  if (window.location.port) {
                    fullUrl = baseUrl + ":" + window.location.port + "/" + url;
                  } else {
                    fullUrl = baseUrl + "/" + url;
                  }
                }
...
InputFieldRenderer.propTypes = {
  ...
  tableName: PropTypes.any,
  inputFieldData: PropTypes.any,
  callSaveFunctionOnLastTab: PropTypes.any,
  handleChange: PropTypes.any,
  onChangeHandler: PropTypes.any,
  onBlurHandler: PropTypes.any,
  formControlValidation: PropTypes.any,
  setStateVariable: PropTypes.any,
  formControlData: PropTypes.any,
  setFormControlData: PropTypes.any,
  isView: PropTypes.any,
  ...
  numberInputStyle: PropTypes.any,
  customDataPickerStyleCss: PropTypes.any,

  // ✅ ADD (these were prop-types errors)
  customTimePickerStyleCss: PropTypes.any,
  customDateTimePickerStyleCss: PropTypes.any,

  dynamicDropDownFieldsData: PropTypes.any,
  dynamicFormData: PropTypes.any,
  clearFlag: PropTypes.any,

  // ✅ ADD (these were no-undef errors)
