...
const getThirdLevelDetails = async (obj) => {
  ...
  if (fetchChargeDetails) {
    ...
    const amount = updatedChargers.reduce(
      (acc, item) => acc + (Number(item["rate"]) || 0) / updatedChargers.length,
      0
    );

    values.tblInvoiceChargeDetails = updatedChargers;
    values["qty"] = qty;
    values["rate"] = amount.toFixed(2);
    values["totalAmountHc"] = (qty * amount).toFixed(2);
    ...
    setStateVariable((prev) => ({
      ...
      totalAmountHc: (qty * amount).toFixed(2),
      totalAmountFc: (
        qty *
        amount *
        Number(newState.exchangeRate || 1)
