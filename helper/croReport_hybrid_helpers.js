...
// The geometry overlap resolver was helping one report but spoiling another
// multi-lane / multi-column style report.
const ENABLE_GEOMETRY_OVERLAP_RESOLVER = false;
...
function getBandKey(el) {
  const b = String(el?.attachBand || "body").toLowerCase();
  return b || "body";
}
...
function getOriginalRectForEl(originalPosMap, el) {
  if (!(originalPosMap instanceof Map) || !el) return null;

  const key = String(
    el?.__flowBaseId || el?.__repeatBaseId || el?.__baseId || el?.id || "",
  );

  return originalPosMap.get(key) || null;
}

function hasStrongOriginalLaneOverlap(a0, b0, ratio = 0.65, minMm = 8) {
  if (!a0 || !b0) return false;

  const overlap =
    Math.min(
      Number(a0.x || 0) + Number(a0.w || 0),
      Number(b0.x || 0) + Number(b0.w || 0),
    ) - Math.max(Number(a0.x || 0), Number(b0.x || 0));

  const minW = Math.max(1, Math.min(Number(a0.w || 0), Number(b0.w || 0)));
  return overlap >= Math.max(minMm, minW * ratio);
}

/**
 * Push elements down when an earlier element has grown in height.
 *
 * Important:
 * - only affects elements in the SAME section
 * - only affects elements in the SAME band
 * - only affects elements originally in the SAME lane
 * - uses original positions/heights as reference
 *
 * This avoids destroying free layouts / multi-column reports.
 */
function applyGrowPushDownWithinContext(
  elements,
  originalPosMap,
  { epsilon = 0.01, debug = true } = {},
) {
  const els = Array.isArray(elements)
    ? elements.map((e) => (e ? { ...e } : e))
    : [];

  if (!els.length || !(originalPosMap instanceof Map) || !originalPosMap.size) {
    return elements;
  }
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const anchorOrig = getOriginalRectForEl(originalPosMap, anchor);
    if (!anchorOrig) continue;

    const delta = Number(anchor.h || 0) - Number(anchorOrig.h || 0);
    const anchorIsSplit = !!(
      anchor?.__flowBaseId ||
      anchor?.__repeatBaseId ||
      anchor?.__baseId
    );

    if (!(delta > epsilon) && !anchorIsSplit) continue;

    const anchorOrigBottom =
      Number(anchorOrig.y || 0) + Number(anchorOrig.h || 0);
    const anchorCurrentBottom = Number(anchor.y || 0) + Number(anchor.h || 0);

    for (let j = i + 1; j < sorted.length; j++) {
      const target = sorted[j];
      if (!target?.id) continue;

      const targetOrig = getOriginalRectForEl(originalPosMap, target);
      if (!targetOrig) continue;

      if (
        String(targetOrig.sectionId || "") !==
        String(anchorOrig.sectionId || "")
      ) {
        continue;
      }
      if (
        String(targetOrig.band || "body") !== String(anchorOrig.band || "body")
      ) {
        continue;
      }

      if (Number(targetOrig.y || 0) + epsilon < anchorOrigBottom) continue;
      if (!hasStrongOriginalLaneOverlap(anchorOrig, targetOrig)) continue;

      const origGap = Math.max(0, Number(targetOrig.y || 0) - anchorOrigBottom);
      const wantY = anchorCurrentBottom + origGap;
      const curY = Number(target.y || 0);

      if (curY < wantY - epsilon) {
        target.y = wantY;
        shifts++;
      }
    }
  ...
  if (debug && shifts) {
    console.log("[GROW-PUSHDOWN-SAFE]", { shifts });
  }

  const byId = new Map(
    sorted.filter((e) => e?.id).map((e) => [String(e.id), e]),
  );
...
function isCollisionCandidate(el) {
  const t = String(el?.type || "").toLowerCase();
  return t === "text" || t === "table" || t === "image";
}

function getRectMm(el) {
...
function hasEnoughHorizontalOverlap(a, b, ratio = 0.3, minMm = 3) {
  const ra = getRectMm(a);
  const rb = getRectMm(b);

  const overlap = Math.min(ra.right, rb.right) - Math.max(ra.x, rb.x);
  const minW = Math.max(1, Math.min(ra.w, rb.w));

  return overlap >= Math.max(minMm, minW * ratio);
}

/**
 * Geometry overlap resolver.
 * Kept for future use, but disabled in pipeline by default because it can
 * disturb free-form multi-column layouts.
 */
function resolveVerticalOverlapsInBands(
  elements,
  { minGapMm = 0.6, maxIters = 80, debug = true } = {},
) {
  const els = Array.isArray(elements)
    ? elements.map((e) => (e ? { ...e } : e))
    : [];
  ...
  for (let iter = 0; iter < maxIters; iter++) {
    ...
    const bandNames = ["header", "body", "footer"];
    ...
    for (const band of bandNames) {
      ...
      for (let i = 0; i < bandEls.length; i++) {
        const a = bandEls[i];
        const aRect = getRectMm(a);
        const wantBottom = aRect.bottom + minGapMm;

        for (let j = i + 1; j < bandEls.length; j++) {
          const b = bandEls[j];
          if (!hasEnoughHorizontalOverlap(a, b)) continue;

          const bY = Number(b?.y || 0);
          if (bY >= wantBottom - 0.001) continue;

          const shift = wantBottom - bY;
          b.y = bY + shift;
          did = true;

          if (debug) {
            console.log("[OVERLAP-RESOLVE]", {
              band,
              anchorId: a?.id,
              movedId: b?.id,
              oldY: bY,
  ...
  const byId = new Map(els.filter((e) => e?.id).map((e) => [String(e.id), e]));
  ...
  try {
    ...
    for (const p of tpl.pages || []) {
      ...
      try {
        ...
        page.elements = applyGrowPushDownWithinContext(
          page.elements,
          __origPosMap,
          { debug },
        );
